...
@Service
@Slf4j
public class AuctionEventConsumerService {
    ...
    @Transactional
    @KafkaListener(topics = "auction-delete-topic", groupId = "bidding-group")
    public void handleAuctionDeletedEvent(AuctionDeletedEvent event) {
        AuctionBidInfo auctionBidInfo = auctionBidInfoRepository.findById(event.getAuctionId())
                .orElseThrow(() -> new AuctionNotFoundException(ErrorCode.AUCTION_NOT_FOUND));
        auctionBidInfoRepository.delete(auctionBidInfo);
    }

    @Transactional
    @KafkaListener(topics = "auction-start-topic", groupId = "bidding-group")
    ...
    // 경매 종료 알림을 받고 상태업데이트
    @Transactional
    @KafkaListener(topics = "auction-close-topic", groupId = "bidding-group")
    ...
    @Transactional
    @KafkaListener(topics = "auction-cancel-topic", groupId = "bidding-group")
